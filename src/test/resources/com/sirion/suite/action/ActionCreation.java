...
public class ActionCreation extends TestSuiteBase implements ITestListener {
	...
	public void checkTestSkip() {
		...
		if (!TestUtil.isTestCaseRunnable(action_suite_xls, this.getClass()
				.getSimpleName())) {
	...
	public void ActionCreationTest(String actionTitle,
			...
			String actionCurrencies, String actionSupplierAccess,
			String actionDependentEntity, String actionTier,
			String actionRequestedOnDate, String actionDueDate,
			String actionFinancialImpact) throws InterruptedException {
		...
		// clicking the contract quick link
		...
		if (!actionType.equalsIgnoreCase("")) {
			new Select(getObject("ac_type_dropdown"))
					.selectByVisibleText(actionType);
		}
		if (!actionPriority.equalsIgnoreCase("")) {
			new Select(getObject("ac_priority_dropdown"))
					.selectByVisibleText(actionPriority);
		}
		if (!actionResponsibility.equalsIgnoreCase("")) {
			new Select(getObject("ac_responsibility_dropdown"))
					.selectByVisibleText(actionResponsibility);
		}
		if (!actionGovernanceBody.equalsIgnoreCase("")) {
			new Select(getObject("ac_govbody_multi"))
					.selectByVisibleText(actionGovernanceBody);// governance
																// body
		}
		if (!actionDeliveryCountries.equalsIgnoreCase("")) {
			new Select(getObject("ac_delcountries_multi"))
					.selectByVisibleText(actionDeliveryCountries);
		}

		if (!actionTimeZone.equalsIgnoreCase("")) {
			new Select(getObject("ac_timezone_dropdown"))
					.selectByVisibleText(actionTimeZone);
		}
		...
		if (!actionCurrencies.equalsIgnoreCase("")) {
			try
			{
			new Select(getObject("ac_currency_dropdown"))
					.selectByVisibleText(actionCurrencies);
			}
			catch(Exception e)
			{
				new Select(getObject("ac_currency_dropdown"))
...
/*		int current_month = date.getMonth();
		...
		driver.findElement(By.name("requestedOn")).click();
		System.out.println("Requested Date Entered");
		...
		WebElement datepicker_ui = driver.findElement(By.xpath("//*[@id='dp1484114058461']"));
...
*/
		/*
		 * Double temp_actionDueDateYear_double =
		 * Double.parseDouble(actionDueDateYear); int temp_actionDueDateYear_int
		 * = temp_actionDueDateYear_double.intValue(); String
		 * actionDueDateYear_string =
		 * Integer.toString(temp_actionDueDateYear_int);
		 * 
		 ...
		 * ).selectByVisibleText(actionDueDateYear_string); }
		 * 
		 * Double temp_actionDueDateMonth_double =
		 * Double.parseDouble(actionDueDateMonth); int
		 * temp_actionDueDateMonth_int =
		 * temp_actionDueDateMonth_double.intValue();
		 * System.out.println(" actionDueDateMonth " +
		 * temp_actionDueDateMonth_int);
		 * 
		 ...
		 * )).click(); } Double temp_actionDueDateDate_double =
		 * Double.parseDouble(actionDueDateDate); int temp_actionDueDateDate_int
		 * = temp_actionDueDateDate_double.intValue(); String
		 * actionDueDateDate_string =
		 * Integer.toString(temp_actionDueDateDate_int);
		 * driver.findElement(By.linkText(actionDueDateDate_string)).click();
		 * 
		 * if (!actionFinancialImpact.equalsIgnoreCase("")) {
		 * getObject("ac_financial_impact_textbox"
		 * ).sendKeys(actionFinancialImpact); }
		 */
		...
		APP_LOGS.debug("Action created successfully with Action id "
				+ action_id);
		...
		// getObject("quick_search_textbox").sendKeys("AC01001"); // created for
		// testing
