...
public class ContractCreditnEarnbackCreation extends TestSuiteBase {
	...
	@Test(groups = "ContractCreditnEarnback", dataProvider = "getTestData")
	  public void ContractCreditnEarnback(String ContractName,String CreditnEarnbackName, String CreditnEarnbackDescription, 
		  ...
		  count++;
		    ...
		    APP_LOGS.debug(" Executing Test Case of Contract Credit and Earnback");
		 ...
		 // Calling method for opening browser from TestBase.java file
		 		    ...
		 		    wait.until(ExpectedConditions.elementToBeClickable(getObject("ceb_create_link_from_contract")));
		 		    getObject("ceb_create_link_from_contract").click(); // click issue create link  
			
			
			
			//Thread.sleep(8000);
			getObject("contract_app_group_name_textbox").clear();
			getObject("contract_app_group_name_textbox").sendKeys(CreditnEarnbackName);
			...
			new Select (driver.findElement(By.xpath("//select[@name='currency']"))).selectByVisibleText(CreditnEarnbackCurrency);
			...
			//Thread.sleep(8000);
			String creditnearnback_show_id = getObject("is_popup_id").getText();
			
			APP_LOGS.debug(" Credit and Earnback"+CreditnEarnbackName+" is created successfully with id "+creditnearnback_show_id);
			//Thread.sleep(5000);
		    ...
		    APP_LOGS.debug(" Quick Search the created contract with Contract id "+creditnearnback_show_id);
		    
		    getObject("quick_search_textbox").sendKeys(creditnearnback_show_id);
		    getObject("quick_search_textbox").sendKeys(Keys.ENTER);
		    //Thread.sleep(8000);
		    APP_LOGS.debug("Contract show page open successfully with contract id " + creditnearnback_show_id);
		    ...
		    driver.findElement(By.xpath("//*[@id='48']/tbody/tr[@role='row']/td[2][contains(.,'"+CreditnEarnbackName+"')]/preceding::td[1]/a")).click();
			  */
		    APP_LOGS.debug("Contract Credit & Earnback show page open successfully with id " + creditnearnback_show_id);
		   /* String CreditnEarnbackIdShowPage = getObject("co_creditnearnback_id_show_page").getText();
		    try
		    {
		    	Assert.assertEquals(CreditnEarnbackIdShowPage, creditnearnback_show_id,"Contract & Earnback ID is -- " +CreditnEarnbackIdShowPage+ " instead of -- " +creditnearnback_show_id);
		    }
		    catch(Throwable e)
		    {
		    	APP_LOGS.debug("Contract & Earnback ID is -- " +CreditnEarnbackIdShowPage+ " instead of -- " +creditnearnback_show_id);
		    }
		    ...
		    fail = false;
		    getObject("analytics_link").click();
            APP_LOGS.debug("Credit and Earnback open successfully, following parameters have been validated: Credit and Earnback name -- " + CreditnEarnbackName);
	  }
