...
@Test(groups = "ContractCreation", dataProvider = "getTestData")
  public void SOWContractsCreation(String parentContractName,String contractName,String contractTitle,String contractAgreement,
		  ...
		  String coComment, String obRequestedBy, String obChangeRequest, String CommentFileUpload) throws InterruptedException {
   ...
   if(!additionalTCV.equalsIgnoreCase("")){
    ...
    getObject("co_general_tab_link").click();
        ...
        if(!coComment.equalsIgnoreCase(""))
        {
        	getObject("co_comment_textbox").sendKeys(coComment);
        }
        
        if(!obRequestedBy.equalsIgnoreCase(""))
        {
        	new Select(getObject("co_requestedby_dropdown")).selectByVisibleText(obRequestedBy);
        }
        
        if(!obChangeRequest.equalsIgnoreCase(""))
        {
        	new Select(getObject("co_changerequest_dropdown")).selectByVisibleText(obChangeRequest);
        }
        
        if(!CommentFileUpload.equalsIgnoreCase(""))
        {
        	getObject("co_comment_file_upload").sendKeys(CommentFileUpload);
        }
       
                getObject("co_save_button").click();
                ...
                Thread.sleep(5000);
                getObject("co_communication_tab").click();
                String documentName=getObject("co_communication_doc_name").getText();
                   
              if(documentName.equalsIgnoreCase(CommentFileUpload))
                {
                	System.out.println("Document Name: "+ documentName +" in communication tab and "+ 
                "File Name that we have uploaded in comment section i.e " + CommentFileUpload +" are same");
                }
