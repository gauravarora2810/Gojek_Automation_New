...
public class BusinessLine extends TestSuiteBase {
	...
	public void BusinessLineCreation (String businessLine, String businessLineActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin Business Line Creation Test -- "+businessLine);
		...
		getObject("sa_business_line_plus_button").click();
		
		getObject("sa_business_line_name_textbox").clear();
		getObject("sa_business_line_name_textbox").sendKeys(businessLine);
		
		getObject("sa_business_line_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_business_line_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_business_line_notification_popup_ok_button").click();
			
			getObject("sa_business_line_cancel_button").click();
        ...
        new Select(getObject("sa_business_line_listing_page_display_dropdown")).selectByIndex(3);
        
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterGroup']/tbody/tr[@role='row']/td[contains(.,'"+ businessLine +"')]/div/a")).click();
        
        String businessLineShowPage = getObject("sa_business_line_show_page_name").getText();
        Assert.assertEquals(businessLineShowPage, businessLine, "Business Line Name at show page is -- " +businessLineShowPage+ " instead of -- " +businessLine);
        
        String businessLineActiveShowPage = getObject("sa_business_line_show_page_active").getText();
        Assert.assertEquals(businessLineActiveShowPage, businessLineActive, "Business Line Active Status at show page is -- " +businessLineActiveShowPage+ " instead of -- " +businessLineActive);
