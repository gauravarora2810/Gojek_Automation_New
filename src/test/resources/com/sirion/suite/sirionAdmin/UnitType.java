...
public class UnitType extends TestSuiteBase {
	...
	public void UnitTypeCreation (String unitType, String unitTypeActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin Unit Type Creation Test -- "+unitType);
		...
		getObject("sa_unit_type_plus_button").click();
		
		getObject("sa_unit_type_name_textbox").clear();
		getObject("sa_unit_type_name_textbox").sendKeys(unitType);
		
		getObject("sa_unit_type_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_unit_type_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_unit_type_notification_popup_ok_button").click();
			
			getObject("sa_unit_type_cancel_button").click();
        ...
        new Select(getObject("sa_unit_type_listing_page_display_dropdown")).selectByIndex(3);
        
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_UnitType']/tbody/tr[@role='row']/td[contains(.,'"+ unitType +"')]/div/a")).click();
        
        String unitTypeShowPage = getObject("sa_unit_type_show_page_name").getText();
        Assert.assertEquals(unitTypeShowPage, unitType, "Unit Type Name at show page is -- " +unitTypeShowPage+ " instead of -- " +unitType);
        
        String unitTypeActiveShowPage = getObject("sa_unit_type_show_page_active").getText();
        Assert.assertEquals(unitTypeActiveShowPage, unitTypeActive, "Unit Type Active Status at show page is -- " +unitTypeActiveShowPage+ " instead of -- " +unitTypeActive);
