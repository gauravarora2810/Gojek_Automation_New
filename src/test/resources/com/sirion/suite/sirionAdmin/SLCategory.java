...
public class SLCategory extends TestSuiteBase {
	...
	public void SLCategoryCreation (String slCategory, String slCategoryActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin SL Category Creation Test -- "+slCategory);
		...
		getObject("sa_sl_category_plus_button").click();
		
		getObject("sa_sl_category_name_textbox").clear();
		getObject("sa_sl_category_name_textbox").sendKeys(slCategory);
		
		getObject("sa_sl_category_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_sl_category_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_sl_category_notification_popup_ok_button").click();
			
			getObject("sa_sl_category_cancel_button").click();
			
			getObject("sa_sl_category_confirmation_popup_yes_button").click();
			
			fail=false;
			APP_LOGS.debug("SL Category already exists with Name -- " +slCategory);
			APP_LOGS.debug("Errors: "+errors_create_page);
			
			return;
			}

        new Select(getObject("sa_sl_category_listing_page_display_dropdown")).selectByIndex(3);
        ...
        String entityTypeNameShowPage = getObject("sa_sl_category_show_page_name").getText();
        Assert.assertEquals(entityTypeNameShowPage, slCategory, "SL Category Name at show page is -- " +entityTypeNameShowPage+ " instead of -- " +slCategory);
        
        String entityTypeActiveShowPage = getObject("sa_sl_category_show_page_active").getText();
        ...
        APP_LOGS.debug("SL Category opened successfully, and following parameters have been validated: SL Category Name -- " +slCategory +", SL Category Active Status -- "+slCategoryActive);
