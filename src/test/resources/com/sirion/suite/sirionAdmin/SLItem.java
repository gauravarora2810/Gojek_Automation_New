...
public class SLItem extends TestSuiteBase {
	...
	public void SLItemCreation (String slItem, String slItemActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin SL Item Creation Test -- "+slItem);
		...
		getObject("sa_sl_item_plus_button").click();
		
		getObject("sa_sl_item_name_textbox").clear();
		getObject("sa_sl_item_name_textbox").sendKeys(slItem);
		
		getObject("sa_sl_item_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_sl_item_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_sl_item_notification_popup_ok_button").click();
			
			getObject("sa_sl_item_cancel_button").click();
        ...
        new Select(getObject("sa_sl_item_listing_page_display_dropdown")).selectByIndex(3);
        
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterSlaitem']/tbody/tr[@role='row']/td[contains(.,'"+ slItem +"')]/a/div")).click();
        
        String entityTypeNameShowPage = getObject("sa_sl_item_show_page_name").getText();
        Assert.assertEquals(entityTypeNameShowPage, slItem, "SL Item Name at show page is -- " +entityTypeNameShowPage+ " instead of -- " +slItem);
        
        String entityTypeActiveShowPage = getObject("sa_sl_item_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, slItemActive, "SL Item Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +slItemActive);
