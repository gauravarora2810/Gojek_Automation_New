...
public class ContractPaper extends TestSuiteBase {
	...
	public void ContractPaperCreation (String contractPaper, String contractPaperActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin Contract Paper Creation Test -- "+contractPaper);
		...
		getObject("sa_contract_paper_plus_button").click();
		
		getObject("sa_contract_paper_name_textbox").clear();
		getObject("sa_contract_paper_name_textbox").sendKeys(contractPaper);
		
		getObject("sa_contract_paper_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_contract_paper_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_contract_paper_notification_popup_ok_button").click();
			
			getObject("sa_contract_paper_cancel_button").click();
			
			getObject("sa_contract_paper_confirmation_popup_yes_button").click();
			
			fail=false;
			APP_LOGS.debug("Contract Paper already exists with Name -- " +contractPaper);
			APP_LOGS.debug("Errors: "+errors_create_page);
			
			return;
			}

        new Select(getObject("sa_contract_paper_listing_page_display_dropdown")).selectByIndex(3);
        ...
        String entityTypeShowPage = getObject("sa_contract_paper_show_page_name").getText();
        ...
        String entityTypeActiveShowPage = getObject("sa_contract_paper_show_page_active").getText();
        ...
        APP_LOGS.debug("Contract Paper opened successfully, and following parameters have been validated: Contract Paper Name -- " +contractPaper +", Contract Paper Active Status -- "+contractPaperActive);
