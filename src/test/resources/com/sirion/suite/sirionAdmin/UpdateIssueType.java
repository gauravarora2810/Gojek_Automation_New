...
public class UpdateIssueType extends TestSuiteBase{
	...
	public void IssueTypeCreation( String issueType, String issueTypeUpdate, String issueTypeActive) throws InterruptedException 
	
	{
		...
		if(!runmodes[count].equalsIgnoreCase("Y")){
			skip=true;
			throw new SkipException("Runmode for test set data set -- " +issueType +" to no " +count);
		}
		...
		APP_LOGS.debug("Executing Sirion Admin Issue Type test -- "+issueType);
		getObject("admin_tab_link").click();
		getObject("sa_issue_type_link").click();
        new Select (getObject("sa_issue_type_display_dropdown")).selectByIndex(3);
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterGroup']/tbody/tr[@role='row']/td[contains(.,'"+ issueType +"')]/div/a")).click();
        //Thread.sleep(5000);
        getObject("sa_issue_type_edit_textbox").click();
        getObject("sa_issue_type_textbox").clear();
        getObject("sa_issue_type_textbox").sendKeys(issueTypeUpdate);
        getObject("sa_issue_type_submit_button").click();
       /*String issueTypeShowPage = getObject("sa_issue_type_show").getText();
       Assert.assertEquals(issueTypeShowPage, issueTypeUpdate, "Issue Type at show page is -- " +issueTypeShowPage+ " instead of -- " +issueTypeUpdate);
       String issueTypeActiveShowPage = getObject("sa_issue_type_active_show").getText();
       Assert.assertEquals(issueTypeActiveShowPage, issueTypeActive, "Issue Type status at show page is -- " +issueTypeActiveShowPage+ " instead of -- " +issueTypeActive);
       */fail = false;
       APP_LOGS.debug("Issue Type open successfully, following parameters have been validated: Issue Type updated name -- " +issueTypeUpdate +
           ", Issue Type status -- "+issueTypeActive);
		getObject("admin_tab_link").click();
