...
public class UpdateWorType extends TestSuiteBase{
	...
	public void OBPhase(String worType, String worTypeUpdte, String worTypeActive) throws InterruptedException 
	
	{
		...
		if(!runmodes[count].equalsIgnoreCase("Y")){
			skip=true;
			throw new SkipException("Runmode for test set data set -- " +worType +" to no " +count);
		}
		...
		APP_LOGS.debug("Executing Sirion Admin WOR Type update test -- "+worType);
		getObject("admin_tab_link").click();
		getObject("sa_wor_type_link").click();
	      new Select (getObject("sa_wor_type_display_dropdown")).selectByIndex(3);
	        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterGroup']/tbody/tr[@role='row']/td[contains(.,'"+ worType +"')]/div/a")).click();
	      //  Thread.sleep(5000);
	        getObject("sa_wor_type_edit_textbox").click();
	        getObject("sa_wor_type_textbox").clear();
	        getObject("sa_wor_type_textbox").sendKeys(worTypeUpdte);
		getObject("sa_wor_type_submit_button").click();

     /*  String worTypeShowPage = getObject("sa_wor_type_show").getText();
       Assert.assertEquals(worTypeShowPage, worTypeUpdte, "WOR Type at show page is -- " +worTypeShowPage+ " instead of -- " +worTypeUpdte);
       String worTypeActiveShowPage = getObject("sa_wor_type_active_show").getText();
       Assert.assertEquals(worTypeActiveShowPage, worTypeActive, "WOR Type status at show page is -- " +worTypeActiveShowPage+ " instead of -- " +worTypeActive);
      */ fail = false;
       APP_LOGS.debug("WOR Type open successfully, following parameters have been validated: WOR Type updated name -- " +worTypeUpdte +
           ", WOR Type status -- "+worTypeActive);
		getObject("admin_tab_link").click();
