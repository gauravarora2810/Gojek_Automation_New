...
public class OBPhase extends TestSuiteBase {
	...
	public void OBPhaseCreation (String obPhase, String obPhaseActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Admin OB Phase Creation Test -- "+obPhase);
		...
		getObject("sa_ob_phase_plus_button").click();
		
		getObject("sa_ob_phase_name_textbox").clear();
		getObject("sa_ob_phase_name_textbox").sendKeys(obPhase);
		
		getObject("sa_ob_phase_save_button").click();

		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			String errors_create_page = getObject("sa_ob_phase_notification_popup_errors_dialogue_box").getText();
			
			getObject("sa_ob_phase_notification_popup_ok_button").click();
			
			getObject("sa_ob_phase_cancel_button").click();
        ...
        new Select(getObject("sa_ob_phase_listing_page_display_dropdown")).selectByIndex(3);
        
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterObligationPhase']/tbody/tr[@role='row']/td[contains(.,'"+ obPhase +"')]/div/a")).click();
        
        String entityTypeShowPage = getObject("sa_ob_phase_show_page_name").getText();
        Assert.assertEquals(entityTypeShowPage, obPhase, "OB Phase Name at show page is -- " +entityTypeShowPage+ " instead of -- " +obPhase);
        
        String entityTypeActiveShowPage = getObject("sa_ob_phase_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, obPhaseActive, "OB Phase Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +obPhaseActive);
