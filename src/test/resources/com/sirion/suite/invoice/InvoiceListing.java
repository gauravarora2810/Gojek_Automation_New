...
public class InvoiceListing extends TestSuiteBase {
  ...
  @Test 
	public void ActionListing() throws InterruptedException, ClassNotFoundException, SQLException
		...
		Thread.sleep(10000);
		numberOfEntries = getObject("action_entries_text").getText();
		
		System.out.println("this is " +numberOfEntries);
		numberOfEntriesSplit = numberOfEntries.split(" ");
		numberOfAction = numberOfEntriesSplit[4];
		System.out.println(numberOfAction);
		
	    Connection con = getDbConnection();
		APP_LOGS.debug("Executing Action listing query");
		String countFromDb = queryResult(con, CONFIG.getProperty("action_listing_query"));
		System.out.println("count from db " +countFromDb);
		
		Assert.assertEquals(numberOfAction, countFromDb);
	    APP_LOGS.debug("Action listing is displaying correct entries.");
	    System.out.println("Action listing is displaying correct entries.");
