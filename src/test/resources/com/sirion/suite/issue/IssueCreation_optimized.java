...
public class IssueCreation_optimized extends TestSuiteBase
{
            ...
            String issueResponsibility, String issueStatus, String issueSupName, String issueSource, String issueSourceName, String issueFunction, String issueService, String issueRegion, String issueCountry) throws InterruptedException
    {
     ...
     // Click on Issue Quick Link   
        getObject("issue_quick_link").click(); //click on contract id
           
        
     //  Click on Issue from which Issue needs to be created again
        
          driver.findElement(By.xpath(".//*[@id='cr']/tbody/tr[1]/td[1]/a")).click();
         
            
          
     // mouse hover on plus button on show page of Issue     
     // Need to give wait before and after the plus button to make mouse hover on plus button to be worked    
          Thread.sleep(10000);
          plus_button("co_plus_button_link");
          Thread.sleep(5000);
     
     // Click on Secondary Issue Link from Issue Show page 
       getObject("is_create_link_from_is").click(); // click issue create link    
           
      
       
    // Title Text Box   
        if(getObject("is_title_textbox").isEnabled()){
    ...
    // Description Text Area Box    
		if (getObject("is_description_textarea").isEnabled()) {
	...
	// Issue Type drop down	
         ...
         if(getObject("is_type_dropdown").isEnabled()){
        	 Select is_type_dropdown_options=new Select(getObject("is_type_dropdown"));
        	 List<WebElement> list_is_type_dropdown_options=is_type_dropdown_options.getOptions();
         for(WebElement temp:list_is_type_dropdown_options)
         {
        	 String temp_to_string=temp.getText();
        	 System.out.println("temp to String is "+ temp_to_string);
        	 System.out.println("Issue type is "+issueType);
        	 if (!issueType.equalsIgnoreCase("") && issueType.equalsIgnoreCase(temp_to_string)) {
        new Select (getObject("is_type_dropdown")).selectByVisibleText(issueType); 
         }
        	 else if(!issueType.equalsIgnoreCase("") && !issueType.equalsIgnoreCase(temp_to_string))
        	 {
        		 new Select (getObject("is_type_dropdown")).selectByVisibleText(temp_to_string);
        	 }
      ...
      // Issue Priority drop down   
         if(getObject("is_priority_dropdown").isEnabled()){
        	 Select is_priority_dropdown_options=new Select(getObject("is_priority_dropdown"));
        	 List<WebElement> list_is_priority_dropdown_options=is_priority_dropdown_options.getOptions();
         for(WebElement temp:list_is_priority_dropdown_options)
         {
        	 String temp_to_string=temp.getText();
        	 System.out.println("temp to String is "+ temp_to_string);
        	 System.out.println("Issue priority is "+issuePriority);
        	 if (!issuePriority.equalsIgnoreCase("") && issuePriority.equalsIgnoreCase(temp_to_string)) {
        new Select (getObject("is_priority_dropdown")).selectByVisibleText(issuePriority); 
         }
        	 else if(!issuePriority.equalsIgnoreCase("") && !issuePriority.equalsIgnoreCase(temp_to_string))
        	 {
        		 new Select (getObject("is_priority_dropdown")).selectByVisibleText(temp_to_string);
        	 }
      ...
      // Issue Currency drop down   
         if(getObject("is_currency_dropdown").isEnabled()){
        	 Select is_currency_dropdown_options=new Select(getObject("is_currency_dropdown"));
        	 List<WebElement> list_is_currency_dropdown_options=is_currency_dropdown_options.getOptions();
         for(WebElement temp:list_is_currency_dropdown_options)
         {
        	 String temp_to_string=temp.getText();
        	 if (!issueCurrencies.equalsIgnoreCase("") && issueCurrencies.equalsIgnoreCase(temp_to_string)) {
        new Select (getObject("is_currency_dropdown")).selectByVisibleText(issueCurrencies); 
         }
        	 else if(!issueCurrencies.equalsIgnoreCase("") && !issueCurrencies.equalsIgnoreCase(temp_to_string))
        	 {
        		 new Select (getObject("is_currency_dropdown")).selectByVisibleText(temp_to_string);
        	 }
       ...
       // Financial Impact text box  
         if(getObject("is_financial_impact_textbox").isEnabled()){
       ...
       // Issue delivery countries multi select
         if(getObject("is_delcountries_multi")!=null){
       ...
       // Issue Time Zone drop down
         if(getObject("is_timezone_dropdown").isEnabled()){
        	 Select is_timezone_dropdown_options=new Select(getObject("is_timezone_dropdown"));
        	 List<WebElement> list_is_timezone_dropdown_options=is_timezone_dropdown_options.getOptions();
         for(WebElement temp:list_is_timezone_dropdown_options)
         {
        	 String temp_to_string=temp.getText();
        	 if (!issueTimeZone.equalsIgnoreCase("") && issueTimeZone.equalsIgnoreCase(temp_to_string)) {
        new Select (getObject("is_timezone_dropdown")).selectByVisibleText(issueTimeZone); 
         }
        	 else if(!issueTimeZone.equalsIgnoreCase("") && !issueTimeZone.equalsIgnoreCase(temp_to_string))
        	 {
        		 new Select (getObject("is_timezone_dropdown")).selectByVisibleText(temp_to_string);
        	 }
      ...
      //   Issue Restricted Access Check box
         if(getObject("is_restrict_puclic_access_checkbox").isEnabled()){
      ...
      // Issue Restricted Required Accesss Checkbox   
         if(getObject("is_restrict_req_access_checkbox").isEnabled()){
      ...
      // Issue Dependentn Entity Check box 
         if(getObject("is_depentity_checkbox").isEnabled()){
      ...
      // Issue Tier Drop down   
        if(getObject("is_tier_dropdown").isEnabled()){
        ...
        // Issue Date 
                   ...
                   // Issue Planned Completion date
                     ...
                     // Issue Responsibility dropdown   
                        if(getObject("is_responsibility_dropdown").isEnabled()){
                        ...
                        driver.findElement(By.xpath("//*[@id='kk']/div/div/div[3]/div[1]/div/form/div[4]/ng-form/div/button[1]")).click();
                      
                        Thread.sleep(4000);
                        String issue_id = getObject("is_popup_id").getText();

                        APP_LOGS.debug("Issue created successfully from Service Level with Issue id " + issue_id);

                        getObject("is_popup_ok_button_from_is").click();
