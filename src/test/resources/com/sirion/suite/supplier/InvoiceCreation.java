...
public class InvoiceCreation extends TestSuiteBase {
 ...
 runmodes = TestUtil.getDataSetRunmodes(supplier_suite_xls, this.getClass().getSimpleName());
  ...
  public void InvCreation(
   ...
   String invPaidAmount, String invResolvedDiscrepancy, String invDisputeAmount
 ...
 APP_LOGS.debug("Executing Test Case Invoice Creation with Invoice title --- " +invTitle+ " under supplier --- ");
 ...
 //driver.findElement(By.xpath("//*[@id='cr']/tbody/tr[@role='row']/td[contains(.,'" "')]/preceding-sibling::td[1]/a")).click();
Thread.sleep(10000);
 plus_button("supplier_plus_button_link"); // web element for plus button on supplier page
 getObject("inv_create_link").click(); // click action create link
 ...
 String invCreditDays_string = Integer.toString(temp_invCreditDays_int);
  ...
  if (!invCreditDays.equalsIgnoreCase("")) {
   getObject("inv_credit_days").sendKeys(invCreditDays_string);
 }
...
/*  if (!supFunctions.equalsIgnoreCase("")) {
    new Select(getObject("inv_sup_functions_multiselect")).selectByVisibleText(supFunctions);
    }
    if (!supServices.equalsIgnoreCase("")) {
        new Select(getObject("inv_sup_services_multiselect")).selectByVisibleText(supServices);
    }
    if (!supRegions.equalsIgnoreCase("")) {
    new Select(getObject("inv_sup_regions_multiselect")).selectByVisibleText(supRegions);
    }
    if (!supCountries.equalsIgnoreCase("")) {
        new Select(getObject("inv_sup_countries_multiselect")).selectByVisibleText(supCountries);
    }
 */
