...
public class SLItem extends TestSuiteBase {
	...
	public void SLItemCreation (String slCategory, String slSubCategory, String slItem, String slItemOp, String slItemActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin SL Item Creation Test -- "+slItemOp);
		...
		getObject("ca_sl_item_plus_button").click();
		
        new Select(getObject("ca_sl_item_dropdown")).selectByVisibleText(slItem);
		
		getObject("ca_sl_item_org_preference_textbox").clear();
		getObject("ca_sl_item_org_preference_textbox").sendKeys(slItemOp);

		if(slItemActive.equalsIgnoreCase("Yes")) {
			getObject("ca_sl_item_active_checkbox").click();
			}
		
		getObject("ca_sl_item_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("SL Item already exists with Name -- " +slItemOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, slItemOp, "SL Item Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +slItemOp);
        ...
        APP_LOGS.debug("SL Item opened successfully, and following parameters have been validated: SL Item Name -- " +slItemOp +", SL Item Active Status -- "+slItemActive);
