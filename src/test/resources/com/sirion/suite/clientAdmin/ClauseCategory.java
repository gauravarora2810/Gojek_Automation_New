...
public class ClauseCategory extends TestSuiteBase {
	...
	public void ClauseCategoryCreation (String clauseCategory, String clauseCategoryOp, String clauseCategoryActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Client Admin Clause Category Creation Test -- "+clauseCategoryOp);
		...
		getObject("ca_clause_category_plus_button").click();
		
        new Select(getObject("ca_clause_category_dropdown")).selectByVisibleText(clauseCategory);
		
		getObject("ca_clause_category_org_preference_textbox").clear();
		getObject("ca_clause_category_org_preference_textbox").sendKeys(clauseCategoryOp);

		if(clauseCategoryActive.equalsIgnoreCase("Yes")) {
			getObject("ca_clause_category_active_checkbox").click();
			}
		
		getObject("ca_clause_category_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("Clause Category already exists with Name -- " +clauseCategoryOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, clauseCategoryOp, "Clause Category Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +clauseCategoryOp);

        String entityTypeActiveShowPage = getObject("ca_clause_category_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, clauseCategoryActive, "Clause Category Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +clauseCategoryActive);
        
        APP_LOGS.debug("Clause Category opened successfully, and following parameters have been validated: Clause Category Name -- " +clauseCategoryOp +", Clause Category Active Status -- "+clauseCategoryActive);
