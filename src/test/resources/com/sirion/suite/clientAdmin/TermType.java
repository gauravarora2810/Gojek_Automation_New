...
public class TermType extends TestSuiteBase {
	...
	public void TermTypeCreation (String termType, String termTypeOp, String termTypeActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin Term Type Creation Test -- "+termTypeOp);
		...
		getObject("ca_term_type_plus_button").click();
		
        new Select(getObject("ca_term_type_dropdown")).selectByVisibleText(termType);
		
		getObject("ca_term_type_org_preference_textbox").clear();
		getObject("ca_term_type_org_preference_textbox").sendKeys(termTypeOp);

		if(termTypeActive.equalsIgnoreCase("Yes")) {
			getObject("ca_term_type_active_checkbox").click();
			}
		
		getObject("ca_term_type_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("Term Type already exists with Name -- " +termTypeOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, termTypeOp, "Term Type Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +termTypeOp);
        ...
        APP_LOGS.debug("Term Type opened successfully, and following parameters have been validated: Term Type Name -- " +termTypeOp +", Term Type Active Status -- "+termTypeActive);
