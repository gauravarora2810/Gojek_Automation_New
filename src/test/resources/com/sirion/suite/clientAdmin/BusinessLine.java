...
public class BusinessLine extends TestSuiteBase {
	...
	public void BusinessLineCreation (String businessLine, String businessLineOp, String businessLineActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin Business Line Creation Test -- "+businessLineOp);
		...
		getObject("ca_business_line_plus_button").click();
		
        new Select(getObject("ca_business_line_dropdown")).selectByVisibleText(businessLine);
		
		getObject("ca_business_line_org_preference_textbox").clear();
		getObject("ca_business_line_org_preference_textbox").sendKeys(businessLineOp);

		if(businessLineActive.equalsIgnoreCase("Yes")) {
			getObject("ca_business_line_active_checkbox").click();
			}
		
		getObject("ca_business_line_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("Business Line already exists with Name -- " +businessLineOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, businessLineOp, "Business Line Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +businessLineOp);

        String entityTypeActiveShowPage = getObject("ca_business_line_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, businessLineActive, "Business Line Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +businessLineActive);
        
        APP_LOGS.debug("Business Line opened successfully, and following parameters have been validated: Business Line Name -- " +businessLineOp +", Business Line Active Status -- "+businessLineActive);
