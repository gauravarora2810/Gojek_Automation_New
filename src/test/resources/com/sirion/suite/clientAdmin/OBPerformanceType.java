...
public class OBPerformanceType extends TestSuiteBase {
	...
	public void OBPerformanceTypeCreation (String obPerformanceType, String obPerformanceTypeOp, String obPerformanceTypeActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin OB Performance Type Creation Test -- "+obPerformanceType);
		...
		getObject("ca_ob_performance_type_plus_button").click();
		
        new Select(getObject("ca_ob_performance_type_dropdown")).selectByVisibleText(obPerformanceType);		
		
		getObject("ca_ob_performance_type_org_preference_textbox").clear();
		getObject("ca_ob_performance_type_org_preference_textbox").sendKeys(obPerformanceTypeOp);

		if(obPerformanceTypeActive.equalsIgnoreCase("Yes")) {
			getObject("ca_ob_performance_type_active_checkbox").click();
			}
		
		getObject("ca_ob_performance_type_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("OB Performance Type already exists with Name -- " +obPerformanceTypeOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, obPerformanceTypeOp, "OB Performance Type Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +obPerformanceTypeOp);
