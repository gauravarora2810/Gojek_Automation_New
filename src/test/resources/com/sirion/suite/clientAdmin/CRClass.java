...
public class CRClass extends TestSuiteBase {
	...
	public void CRClassCreation (String crClass, String crClassOp, String crClassActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin CR Class Creation Test -- "+crClassOp);
		...
		getObject("ca_cr_class_plus_button").click();
		
        new Select(getObject("ca_cr_class_dropdown")).selectByVisibleText(crClass);
		
		getObject("ca_cr_class_org_preference_textbox").clear();
		getObject("ca_cr_class_org_preference_textbox").sendKeys(crClassOp);

		if(crClassActive.equalsIgnoreCase("Yes")) {
			getObject("ca_cr_class_active_checkbox").click();
			}
		
		getObject("ca_cr_class_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("CR Class already exists with Name -- " +crClassOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, crClassOp, "CR Class Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +crClassOp);
        ...
        APP_LOGS.debug("CR Class opened successfully, and following parameters have been validated: CR Class Name -- " +crClassOp +", CR Class Active Status -- "+crClassActive);
