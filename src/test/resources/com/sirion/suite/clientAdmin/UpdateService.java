...
public class UpdateService extends TestSuiteBase {
	...
	public void UpdateServiceTest (String function, String service, String serviceOpUpdate, String serviceActive) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +serviceOpUpdate +" set to NO " +count);
			}
		...
		APP_LOGS.debug("Executing Client Admin Service Update Test -- "+serviceOpUpdate);
        ...
        getObject("ca_service_show_page_edit_button").click();        
        
        getObject("ca_service_org_preference_textbox").clear();
		getObject("ca_service_org_preference_textbox").sendKeys(serviceOpUpdate);
		
		if(!serviceActive.equalsIgnoreCase("Yes")) {
			getObject("ca_service_active_checkbox").click();
			}

		getObject("ca_service_edit_page_update_button").click();
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, serviceOpUpdate, "Service Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +serviceOpUpdate);

        String entityTypeParentShowPage = getObject("ca_service_function_show_page_name").getText();
        Assert.assertEquals(entityTypeParentShowPage, function, "Service Function at show page is -- " +entityTypeParentShowPage+ " instead of -- " +function);
