...
public class ReportContentControls extends TestSuiteBase {
	...
	public void ReportContentControlsTest (String reportName, String status, String performanceStatus) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +reportName +" set to NO " +count);
			}
		
		openBrowser();		
		clientAdminLogin(CONFIG.getProperty("clientAdminURL"), CONFIG.getProperty("clientAdminUsername"), CONFIG.getProperty("clientAdminPassword"));
		
		APP_LOGS.debug("Executing Common Reports Content Controls Test -- "+reportName);
		...
		getObject("ca_report_content_controls_search_textbox").sendKeys(reportName);
		...
		if(driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			APP_LOGS.debug("Quick Link Content Controls Configured Successfully of -- " +reportName);
