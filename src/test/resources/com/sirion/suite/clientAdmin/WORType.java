...
public class WORType extends TestSuiteBase {
	...
	public void WORTypeCreation (String worType, String worTypeOp, String worTypeActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin WOR Type Creation Test -- "+worTypeOp);
		...
		getObject("ca_wor_type_plus_button").click();
		
        new Select(getObject("ca_wor_type_dropdown")).selectByVisibleText(worType);
		
		getObject("ca_wor_type_org_preference_textbox").clear();
		getObject("ca_wor_type_org_preference_textbox").sendKeys(worTypeOp);

		if(worTypeActive.equalsIgnoreCase("Yes")) {
			getObject("ca_wor_type_active_checkbox").click();
			}
		
		getObject("ca_wor_type_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("WOR Type already exists with Name -- " +worTypeOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, worTypeOp, "WOR Type Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +worTypeOp);
        ...
        APP_LOGS.debug("WOR Type opened successfully, and following parameters have been validated: WOR Type Name -- " +worTypeOp +", WOR Type Active Status -- "+worTypeActive);
