...
public class UpdateTransactionType extends TestSuiteBase {
	...
	public void UpdateTransactionTypeTest (String transactionType, String transactionTypeOpUpdate, String transactionTypeActiveUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +transactionTypeOpUpdate +" set to NO " +count);
			}
		...
		APP_LOGS.debug("Executing Client Admin Transaction Type Update Test -- "+transactionTypeOpUpdate);
        ...
        getObject("ca_transaction_type_show_page_edit_button").click();
        
		getObject("ca_transaction_type_org_preference_textbox").clear();
		getObject("ca_transaction_type_org_preference_textbox").sendKeys(transactionTypeOpUpdate);

		if(!transactionTypeActiveUpdate.equalsIgnoreCase("Yes")) {
			getObject("ca_transaction_type_active_checkbox").click();
			}
		
		getObject("ca_transaction_type_edit_page_update_button").click();
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, transactionTypeOpUpdate, "Transaction Type Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +transactionTypeOpUpdate);

        String entityTypeActiveShowPage = getObject("ca_transaction_type_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, transactionTypeActiveUpdate, "Transaction Type Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +transactionTypeActiveUpdate);
