...
public class OBPhase extends TestSuiteBase {
	...
	public void OBPhaseCreation (String obPhase, String obPhaseOp, String obPhaseActive) throws InterruptedException {
		...
		APP_LOGS.debug("Executing Client Admin OB Phase Creation Test -- "+obPhase);
		...
		getObject("ca_ob_phase_plus_button").click();
		
        new Select(getObject("ca_ob_phase_dropdown")).selectByVisibleText(obPhase);
		
		getObject("ca_ob_phase_org_preference_textbox").clear();
		getObject("ca_ob_phase_org_preference_textbox").sendKeys(obPhaseOp);

		if(obPhaseActive.equalsIgnoreCase("Yes")) {
			getObject("ca_ob_phase_active_checkbox").click();
		}
		
		getObject("ca_ob_phase_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("OB Phase already exists with Name -- " +obPhaseOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, obPhaseOp, "OB Phase Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +obPhaseOp);
