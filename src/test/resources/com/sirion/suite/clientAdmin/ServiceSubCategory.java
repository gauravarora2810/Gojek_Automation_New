...
public class ServiceSubCategory extends TestSuiteBase {
	...
	public void ServiceSubCategoryCreation (String serviceCategory, String serviceSubCategory, String serviceSubCategoryOp, String serviceSubCategoryActive) throws InterruptedException {
		count++;
		...
		APP_LOGS.debug("Executing Client Admin Service Sub-Category Creation Test -- "+serviceSubCategoryOp);
		...
		getObject("ca_service_sub_category_plus_button").click();
		
        new Select(getObject("ca_service_sub_category_dropdown")).selectByVisibleText(serviceSubCategory);
		
		getObject("ca_service_sub_category_org_preference_textbox").clear();
		getObject("ca_service_sub_category_org_preference_textbox").sendKeys(serviceSubCategoryOp);

		if(serviceSubCategoryActive.equalsIgnoreCase("Yes")) {
			getObject("ca_service_sub_category_active_checkbox").click();
			}
		
		getObject("ca_service_sub_category_save_button").click();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0) {
			...
			APP_LOGS.debug("Service Sub-Category already exists with Name -- " +serviceSubCategoryOp);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, serviceSubCategoryOp, "Service Sub-Category Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +serviceSubCategoryOp);
        ...
        APP_LOGS.debug("Service Sub-Category opened successfully, and following parameters have been validated: Service Sub-Category Name -- " +serviceSubCategoryOp +", Service Sub-Category Active Status -- "+serviceSubCategoryActive);
