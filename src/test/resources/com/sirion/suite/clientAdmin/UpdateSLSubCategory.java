...
public class UpdateSLSubCategory extends TestSuiteBase {
	...
	public void UpdateSLSubCategoryTest (String slCategory, String slSubCategory, String slSubCategoryOpUpdate, String slSubCategoryActiveUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +slSubCategoryOpUpdate +" set to NO " +count);
			}
		...
		APP_LOGS.debug("Executing Client Admin SL Sub-Category Update Test -- "+slSubCategoryOpUpdate);
        ...
        getObject("ca_sl_sub_category_show_page_edit_button").click();        
        
		getObject("ca_sl_sub_category_org_preference_textbox").clear();
		getObject("ca_sl_sub_category_org_preference_textbox").sendKeys(slSubCategoryOpUpdate);

		if(!slSubCategoryActiveUpdate.equalsIgnoreCase("Yes")) {
			getObject("ca_sl_sub_category_active_checkbox").click();
			}
		
		getObject("ca_sl_sub_category_edit_page_update_button").click();
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, slSubCategoryOpUpdate, "SL Sub-Category Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +slSubCategoryOpUpdate);

        String entityTypeActiveShowPage = getObject("ca_sl_sub_category_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, slSubCategoryActiveUpdate, "SL Sub-Category Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +slSubCategoryActiveUpdate);
