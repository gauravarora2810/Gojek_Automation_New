...
public class UpdatePaperType extends TestSuiteBase {
	...
	public void UpdatePaperTypeTest (String paperType, String paperTypeOpUpdate, String paperTypeActiveUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +paperTypeOpUpdate +" set to NO " +count);
			}
		...
		APP_LOGS.debug("Executing Client Admin Paper Type Creation Test -- "+paperTypeOpUpdate);
        ...
        getObject("ca_paper_type_show_page_edit_button").click();
        
		getObject("ca_paper_type_org_preference_textbox").clear();
		getObject("ca_paper_type_org_preference_textbox").sendKeys(paperTypeOpUpdate);

		if(!paperTypeActiveUpdate.equalsIgnoreCase("Yes")) {
			getObject("ca_paper_type_active_checkbox").click();
			}
		
		getObject("ca_paper_type_edit_page_update_button").click();
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, paperTypeOpUpdate, "Paper Type Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +paperTypeOpUpdate);

        String entityTypeActiveShowPage = getObject("ca_paper_type_show_page_active").getText();
        Assert.assertEquals(entityTypeActiveShowPage, paperTypeActiveUpdate, "Paper Type Active Status at show page is -- " +entityTypeActiveShowPage+ " instead of -- " +paperTypeActiveUpdate);
