...
public class DashboardSetup extends TestSuiteBase {
	...
	public void DashboardSetupTest (String testCaseid, String clientName) throws InterruptedException, TestLinkAPIException {
		count++;
		...
		APP_LOGS.debug("Executing Sirion Client Setup Admin Dashboard Setup Test -- "+clientName);
		...
		new Select(getObject("csa_dashboard_setup_select_client_dropdown")).selectByVisibleText(clientName);
		
		getObject("csa_dashboard_setup_select_chart_button").click();
		getObject("csa_dashboard_setup_select_chart_check_all_checkbox").click();
		
		getObject("csa_dashboard_setup_select_insight_button").click();		
		getObject("csa_dashboard_setup_select_insight_check_all_checkbox").click();
		
		getObject("csa_dashboard_setup_update_button").click();		
		Thread.sleep(2000);		
		
		if (getObject("csa_dashboard_setup_notification_popup_box") != null) {
			String clientShowPage = getObject("csa_dashboard_setup_notification_popup_box").getText();

			Assert.assertEquals(clientShowPage, "Client Configuration Updated Successfully");
			APP_LOGS.debug("Client Dashboard Setup has done Successfully");
			
			getObject("csa_dashboard_setup_notification_popup_ok_button").click();
			}
